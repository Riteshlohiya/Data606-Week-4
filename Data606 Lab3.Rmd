...
#Set my working directory as C:\Users\Arnav\Documents\R\win-library\3.4\DATA606\labs\Lab3\more #
...
Answer: Male height distribution is unimodal and symmetric. Female height distribution is unimodal and left skewed.The mean of the male heights is greater than the mean of female heights. Male height are normally distributed.

```{r}
hist(mdims$hgt, xlab = "Height of Males", main = "Histogram For Males Height")
```

```{r}
hist(fdims$hgt, xlab = "Height of Females", main = "Histogram For Females Height")
```
...
Based on the plot, it does appear that it is nearly normal distribution.
...
Not all the point fall on the line, there are some outliers at the tails. The simulation plot is close to the probability plot
...
Answer: The simulated data flows much more closly to the line. But the normal probability plot for females looks very similar to the simulated data.

Exercise 5: Using the same technique, determine whether or not female weights appear to come from a normal distribution.

Answer: Female weights appear to come from a normal distribution. There are couple of outliers.
...
```{r}
Th_hgt <- 1 - pnorm(q = 150, mean = fhgtmean, sd = fhgtsd)
Th_hgt
```
...
```{r}
Em_hgt <- sum(fdims$hgt > 150) / length(fdims$hgt)
Em_hgt
```

What is the probability that a randomly chosen female is less than 75kg?
...
```{r}
Th_wgt <- pnorm(q = 75, mean = fwgtmean, sd = fwgtsd)
Th_wgt
```
...
```{r}
Em_wgt <- sum(fdims$wgt < 75) / length(fdims$wgt)
Em_wgt
```
...
Answer: As the Age is discrete here instead of integer, C and D have a slight stepwise pattern. 
...
Upward direction suggests that the variable is right skewed and there are outliers on the top end. Please find the Histogram below for confirmation.

```{r}
hist(fdims$kne.di, xlab = "Female Knee Diameter", main = "Histogram for Female Knee Diameter")
```
